<template>
  <div class="q-pa-md" >
    <div class="row q-gutter-y-xs" v-if="computers.length > 0">
        <div class="col-12 col-md-4 col-sm-6 q-gutter-y-xs q-px-xs"
             v-for="(item, index) in computers" :key="index">
          <item :data="computers[index]" @click="logClick" :index="index"/>
        </div>
    </div>
    <div v-else class="">
...
export default class Computers extends Vue {
  ...
  private computers : Array<ProductItem> = []
  ...
  private async filterProducts () : void {
    this.computers = await this.products.filter(
      (item : ProductItem) => {
        return item.category?.name === "Computers"
      }
    )
  }
